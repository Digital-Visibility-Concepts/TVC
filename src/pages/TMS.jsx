...
/* ═══════════════════════════════════════════════════════════════
   TMS QUESTIONNAIRE — added 24 Sep 2026
   All new patient-facing copy lives here so it can be reviewed in
   one place. Any clinical wording needs Dr. Gill's sign-off via
   Bruce before publish.

   ⚠ NAMING: the instrument is the NeuroStar PHQ-10 Patient Survey.
   The URL path says PHQ9 because it is the older path on the same
   Neuronetics application. Do NOT write "PHQ-9" anywhere a patient
   can see it.

   ⚠ NO PHI ON OUR SIDE: this is a link out, opened in a new tab.
   Never iframe it and never rebuild the questions here. The survey
   collects patient name and date under Neuronetics' own HIPAA
   authorisation, and GTM/GA4 run on every page of this site.
   ═══════════════════════════════════════════════════════════════ */

/* Survey 90673, provisioned to Tri-Valley Clinic. Verified 23 Sep 2026.
   Re-confirm it still resolves on build day. */
const SURVEY_URL = "https://phq9web.azurewebsites.net/PHQ9/Survey/90673";

/* ⚠ PENDING Dr. Gill's sign-off. Replace with the approved wording.
   This interim line states only what 988 is and makes no clinical
   claim, so the section is safe to ship if sign-off is late. */
const CRISIS_TEXT =
  "If you are in crisis or thinking about harming yourself, call or text 988 — the Suicide & Crisis Lifeline, available 24/7.";

/* ⚠ PENDING sign-off — the "what happens next" step is operational,
   confirm with the clinic before publish. */
const AFTER_SUBMIT_TEXT =
  "Your responses go directly to Neuronetics and are not collected by this website. Our team will follow up to discuss your results and next steps.";

/* Hero and section image. Client is sending a replacement — swap the
   path here and it updates both places on the page. */
const TMS_IMG = "/assets/tms-img.jpg";

/* GTM container GTM-PPTHL8R9 · GA4 G-ES9FZWTL7Z
   Pushes to the dataLayer only. A trigger on event
   "tms_questionnaire_start" still has to be created in the container,
   with a GA4 event tag pointing at G-ES9FZWTL7Z. Confirm both are
   actually wired before reporting on this. */
function trackQuestionnaireStart() {
  try {
    window.dataLayer = window.dataLayer || [];
    window.dataLayer.push({
      event: "tms_questionnaire_start",
      instrument: "NeuroStar PHQ-10 Patient Survey",
      survey_id: "90673",
      page_path: "/tms-therapy",
    });
...
export default function TMS(){
  ...
  path="/tms-therapy"
/>
<style>{CSS}</style><Cursor/><Hero/><Mq/><WhatIsTMS/><HowWorks/><QuestionnaireSec/><ConsultSec/><FAQSec/><CTA/></main>);
}
function Hero(){
  ...
  return(
    <section className="relative min-h-[88vh] flex items-end overflow-hidden" style={{background:"linear-gradient(140deg,#2C1A0E 0%,#3D2B1F 55%,#4E3525 100%)"}}>
      ...
      <div className="absolute right-0 top-0 bottom-0 w-full lg:w-[52%] overflow-hidden"><img src={TMS_IMG} alt="Clinic" className="w-full h-full object-cover opacity-28" style={{filter:"saturate(0.65)"}}/><div className="absolute inset-0" style={{background:"linear-gradient(to right,#2C1A0E 0%,#2C1A0E 12%,transparent 58%)"}}/><div className="absolute inset-0" style={{background:"linear-gradient(to top,#2C1A0E 0%,transparent 55%)"}}/></div>
...
function WhatIsTMS(){
  ...
  return(<section id="what-is-tms" className="py-24 px-5 md:px-10 bg-[#FDFAF6]"><div className="mx-auto max-w-7xl grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
    <div ref={ref} className={`transition-all duration-800 ${v?"opacity-100 translate-x-0":"opacity-0 -translate-x-10"}`}>
      ...
      <p className="text-[#7A6556] text-base leading-[1.95] font-light mb-8">Treatments are done in-office while you remain awake and alert—no anesthesia, no downtime. Tri-Valley Clinic uses the NeuroStar Advanced Therapy system, which has been FDA-cleared for major depressive disorder since 2008.</p>
    ...
    <div className={`relative transition-all duration-800 delay-200 ${v?"opacity-100 translate-x-0":"opacity-0 translate-x-10"}`}>
      ...
      <div className="overflow-hidden" style={{height:"460px"}}><img src={TMS_IMG} alt="TMS Therapy at Tri-Valley Clinic" className="w-full h-full object-cover transition-transform duration-700 hover:scale-[1.04]" style={{objectPosition:"center top"}}/></div>
...
/* ══ QUESTIONNAIRE ══ Added 24 Sep 2026.
   Light section on purpose: HowWorks above and ConsultSec below are
   both dark, so this also breaks up the dark-on-dark run. */
function QuestionnaireSec(){
  const[ref,v]=useReveal();
  return(<section id="questionnaire" className="py-24 px-5 md:px-10 bg-[#FDFAF6]">
    <div className="mx-auto max-w-4xl">
      <div ref={ref} className={`text-center mb-10 transition-all duration-700 ${v?"opacity-100 translate-y-0":"opacity-0 translate-y-8"}`}>
        ...
        <h2 className="text-5xl md:text-6xl text-[#2C1A0E] mb-5" style={{fontFamily:"'Cormorant Garamond',serif",fontWeight:300}}>Is TMS Right <em className="italic text-[#B8925A]">for You?</em></h2>
        <p className="text-[#7A6556] text-base leading-relaxed font-light max-w-xl mx-auto">Take the NeuroStar PHQ-10 Patient Survey — a short set of questions about mood and daily functioning that helps our team understand where you are before your consultation. It takes just a few minutes.</p>
      </div>

      <div className={`border border-[#E8D5BE] bg-[#F5EEE4] transition-all duration-700 delay-200 ${v?"opacity-100 translate-y-0":"opacity-0 translate-y-8"}`}>
        <div className="h-[3px] bg-gradient-to-r from-transparent via-[#B8925A] to-transparent"/>
        <div className="p-8 md:p-12">
          <div className="grid grid-cols-1 md:grid-cols-[1.15fr_1fr] gap-8 md:gap-10 items-center">

            {/* Left — the survey */}
            <div>
              <p className="text-[10px] tracking-[0.24em] uppercase text-[#B8925A] font-bold mb-3">NeuroStar PHQ-10 Patient Survey</p>
              <p className="text-[#7A6556] text-sm leading-relaxed font-light mb-6">{AFTER_SUBMIT_TEXT}</p>
              <a
                href={SURVEY_URL}
                target="_blank"
                rel="noopener noreferrer"
                onClick={trackQuestionnaireStart}
                className="group inline-flex items-center justify-center gap-3 bg-[#B8925A] text-[#FDFAF6] px-8 py-4 text-[11px] font-bold tracking-[0.2em] uppercase hover:bg-[#C9A46A] transition-colors duration-300 w-full md:w-auto"
              >
                Start the Survey
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M15 3h6v6"/><path d="M10 14 21 3"/><path d="M21 14v5a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5"/></svg>
              </a>
              <p className="text-[10px] tracking-[0.14em] uppercase text-[#7A6556]/70 mt-3">Opens in a new tab &middot; Hosted by Neuronetics</p>
            </div>

            {/* Right — crisis line, beside the button */}
            <div className="border-l-0 md:border-l border-t md:border-t-0 border-[#E8D5BE] pt-6 md:pt-0 md:pl-10">
              <div className="flex items-center gap-2.5 mb-3">
                <span className="w-1.5 h-1.5 rounded-full bg-[#8B3A3A]"/>
                <p className="text-[10px] tracking-[0.2em] uppercase text-[#8B3A3A] font-bold">Need Help Now?</p>
              </div>
              <p className="text-[#7A6556] text-sm leading-relaxed font-light mb-4">{CRISIS_TEXT}</p>
              <a href="tel:988" className="inline-flex items-center gap-2.5 border border-[#8B3A3A]/35 text-[#8B3A3A] px-6 py-3 text-[11px] font-bold tracking-[0.18em] uppercase hover:bg-[#8B3A3A]/6 hover:border-[#8B3A3A]/60 transition-all duration-300">
                <Ph/> Call or Text 988
              </a>
            </div>
      ...
      <p className="text-[11px] text-[#7A6556]/65 font-light mt-5 leading-relaxed text-center max-w-2xl mx-auto">This survey is a screening tool, not a diagnosis. Whether TMS is appropriate for you is determined by your provider during a clinical evaluation.</p>
    </div>
  </section>);
}
...
function FAQSec(){
  ...
  const faqs=[
    {q:"What is the questionnaire on this page?",a:"It is the NeuroStar PHQ-10 Patient Survey — a short set of questions about mood and daily functioning, hosted by Neuronetics, the maker of the NeuroStar system. It helps our team understand where you are before your consultation. Your responses go directly to Neuronetics under their own privacy authorisation and are not collected by this website. After you submit, our team will follow up to discuss your results and next steps. The survey is a screening tool, not a diagnosis — whether TMS is appropriate for you is determined by your provider during a clinical evaluation."},
