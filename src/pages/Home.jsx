...
// /* ══════════════════════════════════════════════════════════════
//    HOMEPAGE POPUPS
//    Sequence: GLP-1 first (4s after load) → on close, 0.5s gap →
//    TMS. No sessionStorage, so the sequence restarts on every
//    page load / refresh.
//    ══════════════════════════════════════════════════════════════ */

// const POPUP_SEQUENCE = ["glp1", "tms"];
// const FIRST_DELAY = 4000;   // ms before the first popup
...
//   useEffect(() => {
//     if (index >= POPUP_SEQUENCE.length) return;
//     const t = setTimeout(() => setOpen(true), index === 0 ? FIRST_DELAY : GAP_DELAY);
...
//   if (!open || index >= POPUP_SEQUENCE.length) return null;
//   const which = POPUP_SEQUENCE[index];

//   /* key={which} forces a fresh mount so the entrance animation and
//      focus handling replay for the second popup. */
//   if (which === "glp1") {
//     return (
//       <HomePopup key="glp1" titleId="glp1-popup-heading" onClose={close}>
...
//   return (
//     <HomePopup key="tms" titleId="tms-popup-heading" onClose={close}>
...
function SpotlightSection() {
  ...
  const SVCS = [
    { n:"01", title:"Psychiatric Care",    tag:"Mental Health",     sub:"Anxiety · Depression · ADHD · PTSD · Bipolar · OCD",           stat:"Accepting New Patients", href:"/psychiatric",  img: IMAGES.DR_GONDARA_WORKING, imgPos:"center 15%", color:"#C9A46A" },
